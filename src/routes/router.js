...
app.post("/receipts", connectEnsureLogin.ensureLoggedIn("/"), (req, res) => {
  const image = req.files.image;

  if (!image) return res.redirect("/receipts");

  // If doesn't have image mime type prevent from uploading
  if (!/^image/.test(image.mimetype)) return res.redirect("/receipts");
...
app.get(
  ...
  async (req, res) => {
    ...
    await itemModel
      .deleteMany({ active: false, age: { $lte: yesterday } })
      .then((obj) => {
    ...
    itemModel
      ...
      .exec()
      .then((items) => {
        res.render("deleted", { page_name: "deleted", items: items });
      });
  }
);
...
app.get("/list", connectEnsureLogin.ensureLoggedIn("/"), async (req, res) => {
  ...
  itemModel
    ...
    .exec()
    .then((items) => {
      res.render("list", { page_name: "list", items: items, total: sum });
    });
});
...
app.post("/list", connectEnsureLogin.ensureLoggedIn("/"), async (req, res) => {
  ...
  try {
    await itemModel.findByIdAndUpdate(deleteId, {
      $set: { active: false, age: new Date() },
    });
  } catch (err) {
...
app.post("/add-item", connectEnsureLogin.ensureLoggedIn("/"), (req, res) => {
  ...
  itemModel.collection.insertOne(
    {
      name: itemName,
      amount: itemAmount,
      price: itemPrice,
